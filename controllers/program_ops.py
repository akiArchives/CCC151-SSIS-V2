...
def delete_program(db, programs_table, parent):
    ...
    deleted_count = 0
    for code in codes:
        # Check if there are students assigned to this program
        query = "SELECT COUNT(*) AS student_count FROM students WHERE program_code = %s"
        result = db.fetch_one(query, (code,))
        if result and result['student_count'] > 0:
            QMessageBox.warning(
                parent,
                "Cannot Delete Program",
                f"Cannot delete program '{code}' because there are students assigned to it."
            )
            continue  # Skip deletion for this program
...
def add_program(db, programs_table, parent):
    ...
    if dialog.exec() == QDialog.DialogCode.Accepted:
        ...
        existing = db.fetch_one("SELECT * FROM programs WHERE code = %s", (program_data['code'],))
        if existing:
...
def edit_program(db, programs_table, parent):
    ...
    if dialog.exec() == QDialog.DialogCode.Accepted:
        program_data = dialog.get_program_data()
        new_code = program_data['code']

        # Check for duplicate program code if it was changed
        if new_code != original_code:
            existing = db.execute_query("SELECT code FROM programs WHERE code = %s", (new_code,))
            if existing:
                QMessageBox.warning(parent, "Duplicate Entry", f"Program code '{new_code}' already exists.")
                return
        ...
        try:
            if db.execute_query(query, (
                ...
                for row in range(programs_table.rowCount()):
                    if programs_table.item(row, 0).text() == new_code:
            ...
            if "Duplicate entry" in str(e):
                QMessageBox.warning(parent, "Duplicate Code", f"Program code '{new_code}' already exists.")
            else:
                QMessageBox.critical(parent, "Database Error", str(e))
